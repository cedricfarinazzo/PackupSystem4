...
uint8_t __SboxGet(size_t x, size_t y)
{
    return sbox[y * 16 + x];
}

uint8_t __InvSboxGet(size_t x, size_t y)
{
    return inv_sbox[y * 16 + x];
}

int __charToint(char c)
{
    if ('0' <= c && c <= '9')
        return (int)(c - '0');
    if ('A' <= c && c <= 'F')
...
struct AES_matrix *AES_matrix_InvSubBytes(struct AES_matrix *block)
{
    ...
    for (size_t y = 0; y < block->rowsLenght; ++y)
    {
        for (size_t x = 0; x < block->colsLenght; ++x)
        {
            ...
            AES_matrix_set(state, x, y, __InvSboxGet(xv, yv));
        }
    }
    return state;
}
